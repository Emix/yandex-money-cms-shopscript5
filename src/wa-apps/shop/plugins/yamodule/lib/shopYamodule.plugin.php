...
class shopYamodulePlugin extends shopPlugin {
    ...
    public function saveSettings($settings = array())
    {
        ...
        if (waRequest::request('ya_kassa_active')) {
            $sm->set('shop.yamodule', 'ya_p2p_active', false);
            $sm->set('shop.yamodule', 'ya_billing_active', false);
            $_POST['ya_p2p_active'] = false;
            $_POST['ya_billing_active'] = false;
        } elseif (waRequest::request('ya_p2p_active')) {
            $sm->set('shop.yamodule', 'ya_billing_active', false);
            $sm->set('shop.yamodule', 'ya_kassa_active', false);
            $_POST['ya_billing_active'] = false;
    ...
    public function kassaOrderReturn()
    {
        ...
        if ($id_order) {
            ...
            if ($mws->CertPem != '') {
                $mws_payment = $mws->request(
                    'listOrders',
                    array(
                        'orderNumber' => implode('/', array('shop', $loaded_plugin['id'], $order['id']))
                    ),
                    false,
                    false
                );
        ...
        $view->assign(array(
            ...
            'taxTotal' => $order['tax'],
            'shipping' => $shipping,
            'ya_kassa_send_check' => isset($data['ya_kassa_send_check']) ? $data['ya_kassa_send_check'] : '',
        ));
